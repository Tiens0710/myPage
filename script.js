...
const defaultHeartImages = Array.from({ length: 1 }, (_, i) => `images/img${i + 1}.jpg`);
...
function createTextRings() {
  ...
  for (let i = 0; i < numRings; i++) {
    ...
    ctx.fillStyle = '#a0a0a0'; // Đổi từ #e0e0e0 thành xám đậm hơn (#a0a0a0) để tối hơn
    ctx.textAlign = 'left';
    ctx.textBaseline = 'alphabetic';

    // Hiệu ứng glow cho viền chữ
    ctx.shadowColor = '#b39ddb'; // Tím đậm hơn, bớt bão hòa so với #d1c4e9
    ctx.shadowBlur = 4; // Giảm từ 12 xuống 8 để glow rất nhẹ
    ctx.lineWidth = 6; // Giảm từ 5 xuống 4 để viền mảnh hơn
    ctx.strokeStyle = '#000000'; // Viền cũng dùng xám đậm hơn
    ctx.strokeText(fullText, 0, textureHeight * 0.82);

    // Hiệu ứng glow cho phần fill
    ctx.shadowColor = '#ff9999'; // Hồng đậm hơn, bớt bão hòa so với #ffc1cc
    ctx.shadowBlur = 10; // Giảm từ 16 xuống 10 để glow nhẹ hơn
    ctx.fillStyle = '#a0a0a0'; // Chữ chính dùng xám đậm hơn
    ...
    ringGroup.userData = {
      ...
      speed: 0.001 + 0.0005, // Tốc độ quay
...
function preloadGalaxyAudio() {
  const audioSources = [
   "https://youtu.be/tcCvZ3MrCxk?si=q5fqgez99mb-VVyR"
  ];
